<x-filament-panels::page>
    <div class="mb-4 flex flex-wrap items-center gap-3">
        <select wire:model.live="projectId"
            ...
            @foreach($this->getProject() ? [$this->getProject()] : [] as $proj)
                        ...
                        <div class="flex-shrink-0 w-64">
                            <div class="flex items-center gap-2 mb-3">
                                ...
                                <span class="text-xs text-gray-400 bg-gray-100 dark:bg-gray-700 rounded-full px-2 py-0.5">
                                    {{ isset($this->getSprintTasks()[$status->id]) ? count($this->getSprintTasks()[$status->id]) : 0 }}
                                </span>
                            </div>
                                    ...
                                    <div class="bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 cursor-grab"
                                        ...
                                        <div class="flex justify-between mt-1">
                                            @if($task->story_points)
                                                <span class="text-xs bg-indigo-100 text-indigo-700 rounded px-1">{{ $task->story_points }} pts</span>
                                            @endif
                                            @if($task->assignee)
            ...
            document.querySelectorAll('.scrum-column').forEach(column => {
                new Sortable(column, {
                    ...
                    onEnd: function (evt) {
                        ...
                        @this.call('moveTask', taskId, statusId);
                    }
                });
            });
        });
