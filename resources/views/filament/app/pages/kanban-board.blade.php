<x-filament-panels::page>
    <div class="mb-4 flex items-center gap-3">
        <select wire:model.live="projectId"
                class="rounded-lg border-gray-300 dark:border-gray-700 dark:bg-gray-800 text-sm focus:ring-2 focus:ring-indigo-500">
        ...
        >
            @foreach($project->taskStatuses as $status)
                <div class="flex-shrink-0 w-72">
                    ...
                    <div
                        class="kanban-column space-y-2 min-h-16 p-2 rounded-xl bg-gray-50 dark:bg-gray-800/50"
                        data-status="{{ $status->id }}"
                    >
                        @foreach($status->tasks->sortBy('position') as $task)
                            <div
                                class="kanban-card bg-white dark:bg-gray-800 rounded-lg p-3 shadow-sm border border-gray-200 dark:border-gray-700 cursor-grab hover:shadow-md transition-shadow"
                                data-task="{{ $task->id }}"
                            >
            ...
            columns.forEach(column => {
                new Sortable(column, {
                    ...
                    onEnd: function (evt) {
                        ...
                        @this.call('moveTask', taskId, statusId, position);
                    }
                });
            });
        });
